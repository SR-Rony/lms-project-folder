...
export function Logo({ className, showTagline = false, compact = false }: LogoProps) {
  ...
  return (
    <Link href="/" className={cn("group flex items-center gap-2.5 lg:gap-3", className)}>
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary text-white shadow-[0_8px_20px_-10px_rgba(255,71,71,0.75)] transition-transform group-hover:scale-105 lg:h-10 lg:w-10 lg:rounded-2xl">
      ...
      <div className={cn("min-w-0 leading-tight", compact && "hidden min-[420px]:block")}>
        <span className="block truncate text-base font-bold tracking-tight text-[#1a1a1a] lg:text-lg">
          {siteConfig.name}
        </span>
        {showTagline ? (
          <span
            className={cn(
              "truncate text-[10px] font-medium text-[#757575] sm:text-xs lg:text-[13px]",
              taglineClassName
            )}
          >
            {siteConfig.tagline}
